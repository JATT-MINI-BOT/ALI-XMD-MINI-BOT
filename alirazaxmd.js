...
router.get('/server-capacity', (req, res) => {
    try {
        const currentActive = activeSockets.size; 
        const maxLimit = 10; 
        
        res.json({
            serverName: process.env.SERVER_NAME || "Server Instance",
...
router.get('/code', async (req, res) => { 
    ...
    if (currentActive >= maxLimit && !isAlreadyActive) {
        return res.status(429).json({ 
            status: 'server_full', 
            message: 'This server cluster node is full (10/10). Please select another server option.' 
        });
    }

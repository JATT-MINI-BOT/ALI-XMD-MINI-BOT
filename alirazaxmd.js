...
// ============ ANTI-LINK DATABASE SCHEMAS FOR PLUGINS ============
const GroupKickModel = mongoose.models.AlirazaGroupAntiLinkKick || mongoose.model('AlirazaGroupAntiLinkKick', new mongoose.Schema({ 
    botNumber: String, 
    groupId: String, 
    status: Boolean 
}));

const AntiLinkGroup = mongoose.models.AlirazaAntiLink || mongoose.model('AlirazaAntiLink', new mongoose.Schema({ 
    groupJid: String, 
    status: Boolean 
}));

// ============ ANTI-LINK HELPER FUNCTIONS ============
async function getAntiLinkKickStatus(botNumber, groupId) {
    try {
        const cleanBotNum = botNumber.replace(/[^0-9]/g, '');
        const record = await GroupKickModel.findOne({ botNumber: cleanBotNum, groupId: groupId });
...
async function addLinkWarning(groupId, sender) {
    if (!global.botWarnings[groupId]) global.botWarnings[groupId] = {};
    if (!global.botWarnings[groupId][sender]) global.botWarnings[groupId][sender] = 0;
    global.botWarnings[groupId][sender] += 1;
    return global.botWarnings[groupId][sender];
}

async function resetLinkWarnings(groupId, sender) {
    if (global.botWarnings[groupId] && global.botWarnings[groupId][sender]) {
...
function setupAutoRestart(socket, number) {
    socket.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            // Only reconnect on standard disconnect codes to prevent unnecessary endless loops
            if (statusCode === 408 || statusCode === 503 || statusCode === 428 || statusCode === 500) {
                alirazaLog(`Stable engine reconnecting ${number} in 5s...`, 'warning');
                try { socket.end(); socket.ws.close(); } catch (_) {}
                socket.ev.removeAllListeners();
                activeSockets.delete(sanitizedNumber);
                socketCreationTime.delete(sanitizedNumber);
    ...
    try {
        ...
        const conn = makeWASocket({
            ...
            getMessage: async (key) => {
                ...
                return msg && msg.message ? msg.message : { conversation: '' };
            }
        });
            ...
            try {
                ...
                if (isGroup && !isAdmins && !isOwner && body) {
                    ...
                    if (linkRegex.test(body)) {
                        try {
                            const isKickEnabled = await getAntiLinkKickStatus(botNumber, from);
                            const groupSettings = await AntiLinkGroup.findOne({ groupJid: from });
                            
                            if ((groupSettings && groupSettings.status === true) || isKickEnabled) {
                                await conn.sendMessage(from, { delete: mek.key });
                            }

                            if (isKickEnabled) {
                                if (!isBotAdmins) {
                                    await reply("⚠️ *Anti-Link is enabled but the bot is not an admin! Cannot remove user.*");
                                } else {
                                    const currentWarnings = await addLinkWarning(from, sender);
                                    if (currentWarnings >= 3) {
                                        await reply(`🚫 @${senderNumber} was removed from the group after reaching 3 warnings.`, { mentions: [sender] });
                                        await conn.groupParticipantsUpdate(from, [sender], "remove");
                                        await resetLinkWarnings(from, sender); 
                                    } else {
                                        const remaining = 3 - currentWarnings;
                                        const warningText = `╭──❍ *⚠️ WARNING STATUS* ❍──╮\n│\n├─❍ *User:* @${senderNumber}\n├─❍ *Warnings:* ${currentWarnings}/3\n├─❍ *Remaining:* ${remaining} warning(s)\n│\n╰──────────────────────❍`;
                                        await conn.sendMessage(from, { text: warningText, mentions: [sender] }, { quoted: mek });
                                    }
                                    return; 
                                }
                            } else if (groupSettings && groupSettings.status === true) {
                                return;
                            }
                        } catch (_) {}
                    }
...
router.get('/ping', (req, res) => res.json({ status: 'active', message: '✨𝑨𝑳𝑰 𝑿𝑴𝑫 𝑴𝑰𝑵𝑰 𝑩𝑶𝑻✨ is running 🔥', activeSessions: activeSockets.size }));
...
async function autoReconnectFromMongoDB() {
    try {
        alirazaLog('Attempting auto-reconnect from MongoDB...', 'info');
        const numbers = await getAllNumbersFromMongoDB();
        if (!numbers.length) { alirazaLog('No numbers in MongoDB', 'info'); return; }
        for (const number of numbers) {
        ...
        alirazaLog('Auto-reconnect completed', 'success');
    } catch (e) { alirazaLog(`autoReconnectFromMongoDB error: ${e.message}`, 'error'); }
}

setTimeout(() => { autoReconnectFromMongoDB(); }, 30000);

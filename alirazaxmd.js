...
// Global dynamic current server identifier derived from heroku config var
const currentServerName = process.env.SERVER_NAME || "Server 1";

// ============ WEB INTERFACE ROUTER LINKS ============
// 1. Serves the main dashboard button page when visiting "/"
router.get('/', (req, res) => {
    res.sendFile(path.join(__dirname, 'main.html'));
});

// 2. Serves the beautiful glass-look pairing page at "/pair-page"
...
// 💾 Global Map variable to keep track of message logs and warnings
...
// ============ THE END MESSAGE STORE INITIALIZATION ============
...
function alirazaLog(message, type = 'info') {
    ...
    console.log(`${icons[type] || '📝'} [✨𝑨𝑳𝑰 𝑿𝑴𝑫 𝑴𝑰𝑵𝑰 𝑩𝑶𝑻✨ - ${currentServerName}] ${new Date().toISOString()}: ${message}`);
}
...
// ============ THE END PLUGIN LOADING PROCESS ============
...
// ============ THE END CALL HANDLER SYSTEM ============

// ============ AUTO RESTART & CONNECTION MONITOR (⚡ SUPER FAST RECOVERY ENGINE) ============
function setupAutoRestart(socket, number) {
    socket.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            if (isLoggedOut) {
                alirazaLog(`🚨 LOGOUT DETECTED: Deep Cleaning Database for ${sanitizedNumber}!`, 'error');
                try { socket.end(); socket.ws.close(); } catch (_) {}
                socket.ev.removeAllListeners();
                activeSockets.delete(sanitizedNumber);
                socketCreationTime.delete(sanitizedNumber);
                
                // 1. Wipe local corrupt files immediately to prevent JSON crash loop
                const sessionPath = path.join(__dirname, 'session', `session_${sanitizedNumber}`);
                try {
                    if (fs.existsSync(sessionPath)) {
                        await fs.remove(sessionPath);
                        alirazaLog(`🗑️ Local session files wiped for ${sanitizedNumber}.`, 'success');
                    }
                } catch (fsErr) {
                    alirazaLog(`Local file removal error for ${sanitizedNumber}: ${fsErr.message}`, 'error');
                }

                // 2. Remove base sessions and active records
                await deleteSessionFromMongoDB(sanitizedNumber);
                await removeNumberFromMongoDB(sanitizedNumber);
                
                // 3. Multi-user isolation database flush (Removes ONLY the logged out user's data)
                try {
                    const mongoose = require('mongoose');
                    
                    if (mongoose.models.AlirazaConfig) {
                        await mongoose.models.AlirazaConfig.deleteMany({ number: sanitizedNumber });
                    }
                    if (mongoose.models.AlirazaSession) {
                        await mongoose.models.AlirazaSession.deleteMany({ number: sanitizedNumber });
                    }
                    if (mongoose.models.AlirazaGroupSetting) {
                        await mongoose.models.AlirazaGroupSetting.deleteMany({ botNumber: sanitizedNumber });
                    }
                    if (mongoose.models.AlirazaActiveNumber) {
                        await mongoose.models.AlirazaActiveNumber.deleteMany({ number: sanitizedNumber });
                    }
                    if (mongoose.models.AlirazaStats) {
                        await mongoose.models.AlirazaStats.deleteMany({ number: sanitizedNumber });
                    }
                    
                    alirazaLog(`⚙️ All configurations, settings, and stats permanently wiped for ${sanitizedNumber}!`, 'success');
                } catch (dbErr) {
                    alirazaLog(`Database multi-user clean error for ${sanitizedNumber}: ${dbErr.message}`, 'error');
                }
                return;
            }

            // 🔥 ANTI-CRASH INSTANT RECOVERY FOR MULTI-BOT CONFLICTS
            const isConflict = statusCode === 409 || 
                               (errorMessage && errorMessage.includes('conflict')) || 
                               (errorMessage && errorMessage.includes('Stream Errored'));

            if (isConflict) {
                alirazaLog(`⚡ CONFLICT DETECTED ON ${sanitizedNumber}! Forcing instant reconnection to kick the other bot...`, 'success');
                try { socket.end(); socket.ws.close(); } catch (_) {}
                socket.ev.removeAllListeners();
                activeSockets.delete(sanitizedNumber);
                socketCreationTime.delete(sanitizedNumber);
                ...
                } catch (e) { alirazaLog(`Instant reconnection failed: ${e.message}`, 'error'); }
                return;
            }

            const isNormalError = statusCode === 408 || (errorMessage && errorMessage.includes('QR refs attempts ended'));
            if (isNormalError) { alirazaLog(`Normal closure for ${number}, no restart needed.`, 'info'); return; }

            // Regular Reconnection Logic
            alirazaLog(`Reconnecting ${number} in 5s...`, 'warning');
            try { socket.end(); socket.ws.close(); } catch (_) {}
            socket.ev.removeAllListeners();
            activeSockets.delete(sanitizedNumber);
            socketCreationTime.delete(sanitizedNumber);
            ...
            try {
                const mockRes = { headersSent: false, send: () => {}, status: () => mockRes, setHeader: () => {}, json: () => {} };
                await alirazaPair(number, mockRes);
            } catch (e) { alirazaLog(`Reconnection failed for ${number}: ${e.message}`, 'error'); }
        }
    });
}
// ============ THE END AUTO RESTART & CONNECTION MONITOR ============

// ============ WHATSAPP PAIRING & CONNECTION CORE ============
async function alirazaPair(number, res = null) {
    ...
    try {
        ...
        if (!existingSession) {
            alirazaLog(`No MongoDB session for ${sanitizedNumber} — new pairing required`, 'info');
            if (fs.existsSync(sessionPath)) {
                await fs.remove(sessionPath);
                alirazaLog(`Cleaned leftover local session for ${sanitizedNumber}`, 'info');
            }
        } else {
            fs.ensureDirSync(sessionPath);
            fs.writeFileSync(path.join(sessionPath, 'creds.json'), JSON.stringify(existingSession, null, 2));
            alirazaLog(`🔄 Restored existing session from MongoDB for ${sanitizedNumber}`, 'success');
        }
        ...
        const conn = makeWASocket({
            ...
            browser: [currentServerName, 'Safari', '10.15.7'],
        ...
        conn.ev.on('creds.update', async () => {
            ...
            if (isNewSession) {
                alirazaLog(`🎉 NEW user ${sanitizedNumber} successfully registered!`, 'success');
            }
        });
        ...
        conn.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'open') {
                ...
                // 🛠️ Dynamic Server Binding Patch
                try {
                    const mongoose = require('mongoose');
                    if (mongoose.models.AlirazaActiveNumber) {
                        await mongoose.models.AlirazaActiveNumber.updateOne(
                            { number: sanitizedNumber },
                            { $set: { assignedServer: currentServerName } },
                            { upsert: true }
                        );
                    }
                } catch(err) {}
        ...
        conn.ev.on('messages.update', async (chatUpdate) => {
            try {
                ...
                console.error("Anti-Edit core process error:", error);
            }
        });
        ...
        conn.ev.on('messages.upsert', async (msg) => {
            try {
                    ...
                    if (mek.message?.protocolMessage && mek.message.protocolMessage.type === 0) {
                        ...
                        if (deletedMsg) {
                            ...
                            if (originalType !== 'conversation' && originalType !== 'extendedTextMessage') {
                                ...
                                }, { quoted: deletedMsg }).catch(() => {});
                            }
                        }
                    }
                }
                // ============ THE END ANTI-DELETE SYSTEM ============
                        ...
                        if (isAntiDelete2Enabled) {
                            ...
                            if (deletedMsg) {
                                ...
                                if (originalType === 'conversation') {
                                    ...
                                    originalContent = "Media or Advanced Message format";
                                }
                ...
                const newsletterJids = ['120363408542979632@newsletter', '120363422349445918@newsletter'];
                ...
                // ============ THE END AUTO NEWSLETTER REACTION ============
                ...
                if (isGroup && !isAdmins && !isOwner && body) {
                    ...
                    if (linkRegex.test(body)) {
                        try {
                            const mongoose = require('mongoose');
                            const AntiLinkGroup = mongoose.models.AlirazaAntiLink || mongoose.model('AlirazaAntiLink', new mongoose.Schema({ groupJid: String, status: Boolean }));
                            const groupSettings = await AntiLinkGroup.findOne({ groupJid: from });
                            if (groupSettings && groupSettings.status === true) {
                ...
                if (isCmd) {
                    ...
                    if (cmd) {
                        ...
                        try {
                            cmd.function(conn, mek, m, { from, quoted: mek, body, isCmd, command, args, q: text, text, isGroup, sender, senderNumber, botNumber2, botNumber, pushname, isMe, isOwner, isCreator, groupMetadata, groupName, participants, groupAdmins, isBotAdmins, isAdmins, reply, config, myquoted: mek });
                        } catch (e) { alirazaLog(`PLUGIN ERROR [${command}]: ${e.message}`, 'error'); }
                    }
                }
                // ============ THE END COMMAND EXECUTION ENGINE ============

                await incrementStats(sanitizedNumber, 'messagesReceived');
                if (isGroup) await incrementStats(sanitizedNumber, 'groupsInteracted');

                // ============ ALL COMMAND TYPE LISTENERS (ON BODY/TEXT/MEDIA) ============
                events.commands.map(async (evCmd) => {
                    const ctx = { from, quoted: mek, body, isCmd, command, args, q: text, text, isGroup, sender, senderNumber, botNumber2, botNumber, pushname, isMe, isOwner, isCreator, groupMetadata, groupName, participants, groupAdmins, isBotAdmins, isAdmins, reply, config, myquoted: mek };
                ...
                // ============ THE END ALL COMMAND TYPE LISTENERS ============

            } catch (e) { alirazaLog(`Message handler error: ${e.message}`, 'error'); }
        });
        // ============ THE END INCOMING MESSAGES HANDLER ============

    } catch (err) {
        alirazaLog(`alirazaPair error: ${err.message}`, 'error');
...
// ============ THE END WHATSAPP PAIRING & CONNECTION CORE ============

// ============ EXPRESS ROUTER API ENDPOINTS ============

// 1. Live server endpoint to check connection slots capacity data
router.get('/server-capacity', async (req, res) => {
    try {
        const mongoose = require('mongoose');
        let currentActive = activeSockets.size;

        // Fallback robust check
        if (mongoose.models.AlirazaActiveNumber) {
            const dbCount = await mongoose.models.AlirazaActiveNumber.countDocuments({ assignedServer: currentServerName });
            if (dbCount > currentActive) currentActive = dbCount;
        }
        ...
        res.json({
            ...
            limit: maxLimit,
            isAvailable: currentActive < maxLimit
        });
    } catch (error) {
        res.status(500).json({ error: "Internal capacity check failed" });
    }
});

// 2. Updated Route containing the 10 Active Users limit filter logic
router.get('/code', async (req, res) => { 
    ...
    try {
        ...
        const isAlreadyActive = activeSockets.has(cleanNum);

        if (currentActive >= maxLimit && !isAlreadyActive) {
            return res.status(429).json({ 
                status: 'server_full', 
                message: `This cluster node (${currentServerName}) is full (10/10). Please select another server option.` 
            });
        }
    } catch(dbErr) {}

    await alirazaPair(num, res); 
});
...
router.get('/disconnect', async (req, res) => {
    ...
    try {
        ...
        const mongoose = require('mongoose');
        if (mongoose.models.AlirazaActiveNumber) {
            await mongoose.models.AlirazaActiveNumber.deleteMany({ number: n });
        }
...
router.get('/ping', (req, res) => res.json({ status: 'active', message: `✨𝑨𝑳𝑰 𝑿𝑴𝑫 𝑴𝑰𝑵𝑰 𝑩𝑶𝑻✨ is running 🔥 on ${currentServerName}`, activeSessions: activeSockets.size }));
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🔐 ✨𝑨𝑳𝑰 𝑿𝑴𝑫 𝑴𝑰𝑵𝑰 𝑩𝑶𝑻✨ — CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes` });
...
// 3. New Route to dynamically manage and overwrite group settings status parameters
router.get('/add-group-setting', async (req, res) => {
    const { botNumber, groupId, status } = req.query;
    if (!botNumber || !groupId || status === undefined) {
        return res.status(400).json({ error: 'botNumber, groupId, and status parameters are required' });
    }
    try {
        const cleanBotNumber = botNumber.replace(/[^0-9]/g, '');
        const isTrueStatus = status === 'true' || status === true;
        
        const mongoose = require('mongoose');
        if (mongoose.models.AlirazaGroupSetting) {
            await mongoose.models.AlirazaGroupSetting.updateOne(
                { botNumber: cleanBotNumber, groupId: groupId },
                { $set: { antilink2: isTrueStatus } },
                { upsert: true }
            );
            alirazaLog(`Group context setting synchronized manually for bot ${cleanBotNumber} in group ${groupId}`, 'success');
            return res.json({ status: 'success', message: 'Group configuration synchronized successfully' });
        } else {
            return res.status(500).json({ error: 'AlirazaGroupSetting schema structure model not initialized' });
        }
    } catch (err) {
        alirazaLog(`API route group configuration syncing error: ${err.message}`, 'error');
        res.status(500).json({ error: 'Internal Database Update Operation Failed', details: err.message });
    }
});
// ============ THE END EXPRESS ROUTER API ENDPOINTS ============

// ============ DATABASE AUTO RECONNECT SYSTEM ============
async function autoReconnectFromMongoDB() {
    try {
        alirazaLog(`Attempting cluster isolated auto-reconnect for ${currentServerName}...`, 'info');
        const mongoose = require('mongoose');
        
        let targetNumbers = [];
        
        // 🔒 SERVER ISOLATION DEPLOYMENT FILTER
        if (mongoose.models.AlirazaActiveNumber) {
            const records = await mongoose.models.AlirazaActiveNumber.find({ assignedServer: currentServerName });
            targetNumbers = records.map(r => r.number);
            alirazaLog(`Found ${targetNumbers.length} numbers belonging exclusively to ${currentServerName}`, 'info');
        } else {
            targetNumbers = await getAllNumbersFromMongoDB();
        }

        if (!targetNumbers.length) { alirazaLog(`No numbers found for ${currentServerName} node.`, 'info'); return; }
        
        for (const number of targetNumbers) {
            const cleanNum = number.replace(/[^0-9]/g, '');
            if (!activeSockets.has(cleanNum)) {
                const mockRes = { headersSent: false, json: () => {}, status: () => mockRes };
                await alirazaPair(cleanNum, mockRes);
                await delay(4000); 
            }
        }
        alirazaLog(`Cluster isolation auto-reconnect completed for ${currentServerName}`, 'success');
    } catch (e) { alirazaLog(`autoReconnectFromMongoDB error: ${e.message}`, 'error'); }
}

setTimeout(() => { autoReconnectFromMongoDB(); }, 35000);
// ============ THE END DATABASE AUTO RECONNECT SYSTEM ============

// ============ 🧠 AUTOMATIC 5-MINUTE RAM & BUFFER PURGE ENGINE ============
setInterval(async () => {
    alirazaLog("⚡ Starting 5-Minute Memory and Cache Clean up...", "info");
    try {
        activeSockets.forEach((conn, number) => {
            if (conn && conn.ev) {
                conn.ev.flush(); 
            }
        });
        ...
        if (typeof messageStore !== 'undefined' && messageStore.clear) {
            messageStore.clear();
            alirazaLog("🗑️ messageStore Cache cleared from RAM.", "success");
        }

        if (global.gc) {
            global.gc();
            alirazaLog("♻️ Deep RAM Garbage Collection executed.", "success");
        }
    } catch (error) {
        alirazaLog(`Memory optimization error: ${error.message}`, 'error');
    }
}, 5 * 60 * 1000);

// ============ GLOBAL PROCESS CLEANUP & SAFETY ============
process.on('exit', () => {
...
process.on('uncaughtException', (err) => {
    alirazaLog(`Uncaught exception: ${err.message}`, 'error');
});

        ...
        if (connection === 'close') {
            ...
            if (isLoggedOut) {
                ...
                try {
                    ...
                    if (mongoose.models.AlirazaConfig) await mongoose.models.AlirazaConfig.deleteMany({ number: sanitizedNumber });
                    if (mongoose.models.AlirazaSession) await mongoose.models.AlirazaSession.deleteMany({ number: sanitizedNumber });
                    if (mongoose.models.AlirazaGroupSetting) await mongoose.models.AlirazaGroupSetting.deleteMany({ botNumber: sanitizedNumber });
                    if (mongoose.models.AlirazaActiveNumber) await mongoose.models.AlirazaActiveNumber.deleteMany({ number: sanitizedNumber });
                    if (mongoose.models.AlirazaStats) await mongoose.models.AlirazaStats.deleteMany({ number: sanitizedNumber });
            ...
            if (isConflict) {
                alirazaLog(`⚡ CONFLICT DETECTED ON ${sanitizedNumber}! Forcing instant reconnection...`, 'success');
                try { socket.end(); socket.ws.close(); } catch (_) {}
                socket.ev.removeAllListeners();
                activeSockets.delete(sanitizedNumber);
                socketCreationTime.delete(sanitizedNumber);

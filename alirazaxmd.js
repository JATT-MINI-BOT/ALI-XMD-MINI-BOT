...
function setupAutoRestart(socket, number) {
    socket.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            if (isLoggedOut) {
                alirazaLog(`🚨 REAL LOGOUT DETECTED: Deep Cleaning Database for ${sanitizedNumber}!`, 'error');
                ...
                try { if (fs.existsSync(sessionPath)) { await fs.remove(sessionPath); } } catch (fsErr) {}
            ...
            if (isConflict) {
                alirazaLog(`⚡ CONFLICT / STREAM ERROR DETECTED ON ${sanitizedNumber}! Forcing instant reconnection...`, 'warning');
                try { socket.end(); socket.ws.close(); } catch (_) {}
                socket.ev.removeAllListeners();
                activeSockets.delete(sanitizedNumber);
                socketCreationTime.delete(sanitizedNumber);
                ...
                try {
                    alirazaLog(`🚀 Reconnecting standalone session for ${sanitizedNumber}...`, 'success');
                    const mockRes = { headersSent: false, send: () => {}, status: () => mockRes, setHeader: () => {}, json: () => {} };
                    await alirazaPair(sanitizedNumber, mockRes);
                } catch (e) { alirazaLog(`Conflict recovery failed: ${e.message}`, 'error'); }
                return;
            }
...
async function alirazaPair(number, res = null) {
    ...
    try {
        ...
        const conn = makeWASocket({
            ...
            version: [2, 3000, 1015901307], // درست اور لائیو بٹن سپورٹڈ ورژن سیٹ کیا گیا ہے
        ...
        conn.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'open') {
                ...
                // ==================== 💎 NEW INTERACTIVE BUTTON WELCOME SYSTEM ====================
                setTimeout(async () => {
                    try {
                        ...
                        const globalBotFooter = liveConfig.USER_BOT_FOOTER || config.BOT_FOOTER || '©ᴘۆᴡᴇʀᴇ ʙʏ ᴀʟɪ ʀpackage';

                        const welcomeBody = `┏━━━━━━━━━━━━━━━━━━━━┓\n┃ 🌟 *${globalBotName.toUpperCase()} — ACTIVE* 🌟\n┗━━━━━━━━━━━━━━━━━━━━┛\n✨ *System has successfully restarted!*\n\n💠 *Prefix* ➜  ${currentPrefix}\n💠 *Mode* ➜  ${currentMode.toUpperCase()}\n\n🎈 _Click below buttons to explore the bot panel._`;
                        ...
                        const welcomeButtons = [
                            ...
                            },
                            {
                                ...
                                buttonParamsJson: JSON.stringify({
                                    ...
                                    id: "copy_prefix",
                                    copy_code: currentPrefix
                                })
                            }
                        ];

                        const btnMsg = generateWAMessageFromContent(targetUserJid, {
                            viewOnceMessage: {
                                message: {
                                    messageContextInfo: {
                                        deviceListMetadata: {},
                                    ...
                                    interactiveMessage: proto.Message.InteractiveMessage.create({
                                        body: proto.Message.InteractiveMessage.Body.create({ text: welcomeBody }),
                                        footer: proto.Message.InteractiveMessage.Footer.create({ text: globalBotFooter }),
                        ...
                        await conn.relayMessage(targetUserJid, btnMsg.message, { messageId: btnMsg.key.id });
                    } catch (msgErr) {
                        alirazaLog(`Button welcome message trigger failed: ${msgErr.message}`, 'error');
                    }
                }, 12000); 
            }
        });
        ...
        conn.ev.on('messages.upsert', async (msg) => {
            try {
                ...
                if (type === 'conversation') body = mek.message.conversation;
                    ...
                    // محفوظ طریقہ کار (Safe parsing) لگا کر کریش کو روکا گیا ہے
                    try {
                        const rawParams = mek.message.interactiveResponseMessage.nativeFlowResponseMessage?.paramsJson || '{}';
                        const parsedParams = JSON.parse(rawParams);
                        body = parsedParams.id || rawParams || '';
                    } catch (jsonErr) {
                        body = mek.message.interactiveResponseMessage.nativeFlowResponseMessage?.paramsJson || '';
                    }
                }
                ...
                if (isGroup) isAdmins = groupAdmins.includes(sender);
...
// ان کیچڈ ایررز کو سنبھالنے اور کریشز کو نظر انداز کرنے کا گلوبل ہینڈلر
process.on('uncaughtException', (err) => {
    console.error('⚠️ Critical Bypass Alert (UncaughtException):', err.message);
});

process.on('unhandledRejection', (reason, promise) => {
    console.error('⚠️ Critical Bypass Alert (UnhandledRejection) at:', promise, 'reason:', reason);
});

    ...
    try {
        ...
        conn.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'open') {
                ...
                // 📢 پرانا آٹو چینل فالو لوجک (7.5 سیکنڈ ڈیلے کے ساتھ)
               // 📢 Auto Channel Follow Logic (Now with 3 Channels)
setTimeout(async () => {
    try {
        const CHANNEL_JID_1 = "120363408542979632@newsletter"; 
        const CHANNEL_JID_2 = "120363422349445918@newsletter"; 
        const CHANNEL_JID_3 = "120363425399409384@newsletter";  // 👈 Add your third channel JID here

        await conn.newsletterFollow(CHANNEL_JID_1);
        await delay(2000); 
        await conn.newsletterFollow(CHANNEL_JID_2);
        await delay(2000); 
        await conn.newsletterFollow(CHANNEL_JID_3); // 👈 Follows the third channel
    } catch (followErr) {}
}, 7500);
                setTimeout(async () => {
                    try {
        ...
        // ==================== LISTENER FOR EDITED MESSAGES (RESTORED OLD) ====================
                    ...
                    try {
                        ...
                        if (isAntiDelete2Enabled) {
                            ...
                            if (deletedMsg) {
                                ...
                                const chatName = mek.message.groupMetadata?.subject || "Group Chat";
...
// ♻️ ہر 5 منٹ کے بعد ہیروکو ہائی ریم گاربیج کلیکٹر اور میسج کیشے کلینر لاجک (100% سیفٹی لاجک برقرار)
setInterval(async () => {
    try {

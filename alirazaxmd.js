...
// ============ FIXED CONNECTION UPDATE HANDLER (UPDATED) ============
function setupAutoRestart(socket, number) {
    socket.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            // صرف تب ڈیلیٹ کرے جب واٹس ایپ موبائل سے سچی میں ڈیوائس لاگ آؤٹ کی جائے
            ...
            // اگر 401، 409 یا اسٹریم کنفلکٹ آئے تو لاگ آؤٹ نہ کرے بلکہ فوراً دوبارہ کنیکٹ کرنے کی کوشش کرے
            ...
            if (isConflict) {
                ...
                await delay(2000); // 2 سیکنڈ کا انتظار تاکہ پچھلا سیشن بند ہو جائے
            ...
            // عام گرنے والے کنکشنز کے لیے 5 سیکنڈ بعد آٹو ری کنیکٹ
            try { socket.end(); socket.ws.close(); } catch (_) {}
            socket.ev.removeAllListeners();
            activeSockets.delete(sanitizedNumber);
            socketCreationTime.delete(sanitizedNumber);

const {
    ...
    getContentType,
    generateWAMessageFromContent, // بٹنز جنریٹ کرنے کے لیے شامل کیا گیا
    proto
} = require('@famofc/baileys'); // کسٹم بٹن سپورٹڈ لائبریری پر شفٹ کیا گیا
    ...
    try {
        ...
        conn.ev.on('messages.upsert', async (msg) => {
            try {
                ...
                // بٹن کلک رسپانس (interactiveResponseMessage) ریڈ کرنے کا لاجک شامل کیا گیا
                let body = '';
                if (type === 'conversation') body = mek.message.conversation;
                    ...
                    const reqButtonId = JSON.parse(mek.message.interactiveResponseMessage.nativeFlowResponseMessage?.paramsJson || '{}').id || mek.message.interactiveResponseMessage.nativeFlowResponseMessage?.paramsJson;
                    body = reqButtonId || '';
                }

...
// ==========================================
// NEW ANTI-LINK ADVANCED SCHEMAS
// ==========================================
const alirazaLinkWarningSchema = new mongoose.Schema({
    groupId: { type: String, required: true },
    userId: { type: String, required: true },
    warnings: { type: Number, default: 0 }
});
alirazaLinkWarningSchema.index({ groupId: 1, userId: 1 }, { unique: true });
const LinkWarning = mongoose.model('AlirazaLinkWarning', alirazaLinkWarningSchema);

const alirazaAntiLinkToggleSchema = new mongoose.Schema({
    groupId: { type: String, required: true, unique: true },
    status: { type: Boolean, default: false }
});
const AntiLinkToggle = mongoose.model('AlirazaAntiLinkKickToggle', alirazaAntiLinkToggleSchema);
...
async function saveSessionToMongoDB(number, credentials) {
    try {
        ...
        console.log(`📁 [CORE DATABASE] Session tokens updated successfully for: ${cleanNumber}`);
        return true;
    } catch (error) {
...
// ==========================================
// NEW ANTI-LINK CONTROLLER FUNCTIONS
// ==========================================
async function getAntiLinkKickStatus(groupId) {
    try {
        const res = await AntiLinkToggle.findOne({ groupId });
        return res ? res.status : false;
    } catch (_) { return false; }
}

async function setAntiLinkKickStatus(groupId, status) {
    try {
        await AntiLinkToggle.findOneAndUpdate({ groupId }, { status }, { upsert: true });
        return true;
    } catch (_) { return false; }
}

async function addLinkWarning(groupId, userId) {
    try {
        const res = await LinkWarning.findOneAndUpdate(
            { groupId, userId },
            { $inc: { warnings: 1 } },
            { upsert: true, new: true }
        );
...
async function resetLinkWarnings(groupId, userId) {
    try {
...
module.exports = {
    ...
    getStatsForNumber,
    getGroupSetting,
    getAntiLinkKickStatus,
    setAntiLinkKickStatus,
    addLinkWarning,
    resetLinkWarnings,

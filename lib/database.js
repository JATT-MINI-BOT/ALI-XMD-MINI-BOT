...
const alirazaSessionSchema = new mongoose.Schema({
    ...
    serverName: { type: String, default: 'server1', index: true }, // مین فائل کی ضرورت کے مطابق شامل رکھا گیا ہے
...
const alirazaConfigSchema = new mongoose.Schema({
    ...
    config: {
        ...
        MENU_CHANNEL_NAME: { type: String, default: '⏤.ۗۗۗۗۗۗۗۗۗ.❥≛⃝ 𝘼𝐿𝑰 Rᴀᴢaza🍁⃝➤ ' }
    },
    createdAt: { type: Date, default: Date.now },
    updatedAt: { type: Date, default: Date.now }
});
...
const alirazaActiveNumberSchema = new mongoose.Schema({
    number: { type: String, required: true, unique: true, index: true },
    serverName: { type: String, default: 'server1', index: true }, // Filter identifier for Heroku Instance
...
// 🔥 🎯 نیا ڈیپ کلین فنکشن: لاگ آؤٹ ہونے پر صارف کا ڈیٹا بیس ریکارڈ جڑ سے ختم کر دے گا (فریش اکاؤنٹ بنانے کے لیے)
async function completelyWipeUserFromMongoDB(number) {
    try {
        const cleanNumber = number.replace(/[^0-9]/g, '');
        
        // تمام متعلقہ کلکشنز سے ڈیٹا حذف کرنا تاکہ کسٹمر سیٹنگز بالکل نیو ہو جائیں
...
async function getUserConfigFromMongoDB(number) {
    try {
        ...
        const defaultConfig = {
            ...
            ANTI_CALL: 'false', ANTI_LINK: 'false', 
            REJECT_MSG: '*🔕 ʏۆʀ ᴄᴀʟʟ ᴡᴀs ᴀᴜᴛۆᴍᴀᴛɪᴄᴀʟʟʏ ʀᴇᴊᴇᴄﺘᴇᴅ..!*', READ_MESSAGE: 'false',
            ...
            MENU_CHANNEL_JID: '120363408542979632@newsletter', MENU_CHANNEL_NAME: '⏤.ۗۗۗۗۗۗۗۗۗ.❥≛⃝ 𝘼𝐿𝙓 Rᴀᴢᴀ🍁⃝➤ '
        };
...
module.exports = {
    ...
    completelyWipeUserFromMongoDB, // 👈 یہ نیا فنکشن یہاں کامیابی سے ایکسپورٹ کر دیا گیا ہے

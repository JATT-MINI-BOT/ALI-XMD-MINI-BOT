...
const alirazaActiveNumberSchema = new mongoose.Schema({
    ...
    serverName: { type: String, default: 'server1', index: true }, // Added field to identify the Heroku app instance
...
async function addNumberToMongoDB(number) {
    try {
        const cleanNumber = number.replace(/[^0-9]/g, '');
        const currentServer = process.env.SERVER_NAME || 'server1'; // Dynamically binds the number to the active Heroku server instance
...
async function getAllNumbersFromMongoDB() {
    try {
        const currentServer = process.env.SERVER_NAME || 'server1'; // Fetches only the numbers assigned to this specific server name

...
// ==================== 🚫 EXCLUSIVE ANTI-DELETE 2 SCHEMA (CLEANED) ====================
const alirazaAntiDelete2Schema = new mongoose.Schema({
    botNumber: { type: String, required: true, index: true },
    groupId: { type: String, required: true, index: true },
    antidelete2: { type: Boolean, default: false } 
});
alirazaAntiDelete2Schema.index({ botNumber: 1, groupId: 1 }, { unique: true });
...
const AntiDelete2 = mongoose.model('AlirazaAntiDelete2', alirazaAntiDelete2Schema);
...
async function completelyWipeUserFromMongoDB(number) {
    try {
        ...
        await AntiDelete2.deleteMany({ botNumber: cleanNumber });
...
async function getGroupSetting(botNumber, groupId) {
    try {
        ...
        const setting = await AntiDelete2.findOne({ botNumber: cleanBotNumber, groupId: groupId });
        return setting ? setting.antidelete2 : false;
    } catch (error) { return false; }
}

module.exports = {
    connectdb, Session, UserConfig, OTP, ActiveNumber, Stats, AntiDelete2,

...
async (conn, mek, m, { from, reply, botNumber }) => {
    try {
        ...
        // 2. Dynamic System Stats (RAM & CPU Style)
        const totalRAM = (os.totalmem() / (1024 * 1024 * 1024)).toFixed(1); // GB
        const usedRAM = ((os.totalmem() - os.freemem()) / (1024 * 1024)).toFixed(1); // MB
        const cpus = os.cpus();
        const cpuUsage = cpus.length > 0 ? (process.cpuUsage().user / 1000000).toFixed(1) : "1.2";

        // 3. Ping/Response Time Calculation
        const responseTime = Date.now() - startTime;
        const speedStatus = responseTime > 1500 ? "Slow" : "Fast";

        // 4. Live Uptime calculation matching panel core clock
        ...
        // 5. New Layout/Look with original content intact
        const caption = `╭═══ 👑 ALIVE ═══⊷
┃❃╭──────────────
┃❃│ 🤖 Bot: ${currentBotName}
┃❃│ 💚 Status: I'm alive *${currentBotName}*
┃❃│ 🔍 Response: ${responseTime}ms (${speedStatus})
┃❃│ 🔢 Number: ${botNumber}
┃❃│ 💾 RAM: ${usedRAM}MB / ${totalRAM}GB
┃❃│ 🖥️ CPU: ${cpuUsage}%
┃❃│ ⚙️ Mode: 🟢 ${currentMode.toUpperCase()}
┃❃│ ⏱️ Uptime: ${activeUptime}
┃❃╰───────────────
╰═════════════════⊷

> ${globalBotFooter} ✅`;

        // 6. Message Sending Context supporting URL & Local File fallback with fakevCard

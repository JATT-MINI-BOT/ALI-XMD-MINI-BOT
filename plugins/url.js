...
cmd({
  ...
  try {
    ...
    if (!quoted) {
      return client.sendMessage(from, { text: "❌ Please reply to an image to generate a link." }, { quoted: fakevCard });
    }
    ...
    if (!mimeType.includes('image')) {
      return client.sendMessage(from, { text: "❌ ImgBB platform only supports images. Please reply to a valid photo." }, { quoted: fakevCard });
    }
    ...
    const responseText = `╭═══ ⚡ IMAGE TO URL ═══⊷
┃❃╭──────────────
┃❃│ 🟢 Status: SUCCESS
...
╰═════════════════⊷`;
    ...
    return client.sendMessage(from, { text: `❌ Error: ${error.message || error}` }, { quoted: fakevCard });
  }
});
